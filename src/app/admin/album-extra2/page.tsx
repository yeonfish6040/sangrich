...
export default async function AdminAlbumExtra2Page() {
  const session = await requireAuth();
  await dbConnect();

  // admin은 모든 글, user는 자기가 쓴 글만
  const query = session.role === 'admin' ? {} : { createdBy: session.username };
  const albums = await AlbumExtra2.find(query).sort({ createdAt: -1 }).lean();
        ...
        <div className="mx-auto max-w-7xl px-6 py-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h1 className="text-3xl font-bold text-gray-900">앨범 Extra 2 관리</h1>
            </div>
            <Link
              href="/admin/album-extra2/new"
                ...
                albums.map((album) => (
                  <tr key={album._id.toString()} className="hover:bg-gray-50">
                    ...
                    <td className="whitespace-nowrap px-6 py-4 text-right text-sm font-medium">
                      <Link href={`/admin/album-extra2/${album._id.toString()}`} className="mr-3 text-blue-600 hover:text-blue-900">수정</Link>
