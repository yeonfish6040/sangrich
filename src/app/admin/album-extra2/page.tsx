...
export default function AdminAlbumExtra2Page() {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/album-extra2/${id}`, { method: 'DELETE' });
      if (res.ok) {
        alert('삭제되었습니다.');
        fetchAlbums();
      } else {
                ...
                albums.map((album) => (
                  <tr key={album._id} className="hover:bg-gray-50">
                    ...
                    <td className="whitespace-nowrap px-6 py-4 text-right text-sm font-medium">
                      <Link href={`/admin/album-extra2/${album._id}`} className="mr-3 text-blue-600 hover:text-blue-900">수정</Link>
                      <button
