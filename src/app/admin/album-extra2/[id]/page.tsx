...
export default function EditAlbumExtra2Page({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/album-extra2/${id}`, {
        method: 'PUT',
    ...
    <div className="min-h-screen bg-gray-100">
      <div className="bg-white shadow">
        <div className="mx-auto max-w-7xl px-6 py-6">
          ...
          <h1 className="text-3xl font-bold text-gray-900">앨범 수정 (Extra 2)</h1>
        </div>
      </div>
