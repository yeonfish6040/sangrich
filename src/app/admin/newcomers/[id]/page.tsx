...
export default function EditNewComerPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/newcomers/${id}`, {
        method: 'PUT',
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/newcomers/${id}`, { method: 'DELETE' });
      if (res.ok) {
        alert('새신자가 삭제되었습니다.');
        router.push('/admin/newcomers');
      }
    ...
    <div className="min-h-screen bg-gray-100">
      <div className="bg-white shadow">
        <div className="mx-auto max-w-7xl px-6 py-6">
          ...
          <h1 className="text-3xl font-bold text-gray-900">새신자 수정</h1>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-3xl px-6 py-10">
        <form onSubmit={handleSubmit} className="rounded-lg bg-white p-6 shadow">
          <div className="space-y-6">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700">사진 추가</label>
              <input
                type="file"
                accept="image/*"
                multiple
