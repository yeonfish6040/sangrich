...
export default async function BoardExtra1Page({ searchParams }: PageProps) {
  ...
  // Get total count
  const totalItems = await BoardExtra1.countDocuments(filter);
  const totalPages = Math.ceil(totalItems / itemsPerPage);

  // Fetch items for current page
  const boards = await BoardExtra1.find(filter)
    .sort({ createdAt: -1 })
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-6 py-10">
      <h1 className="mb-6 text-3xl font-bold">게시판 Extra 1</h1>

      {/* Search bar */}
      <div className="mb-6 flex gap-2">
        <form className="flex w-full gap-2" action="/board-extra1" method="get">
          <select
            name="searchType"
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-sm">
                      <Link
                        href={`/board-extra1/${board._id.toString()}`}
      ...
      {totalPages > 0 && (
        <div className="mt-6 flex items-center justify-center gap-2">
          ...
          {currentPage > 1 && (
            <a
              href={`/board-extra1?page=1${searchQuery ? `&search=${searchQuery}&searchType=${searchType}` : ''}`}
          ...
          {currentPage > 1 && (
            <a
              href={`/board-extra1?page=${currentPage - 1}${searchQuery ? `&search=${searchQuery}&searchType=${searchType}` : ''}`}
              className="rounded border px-3 py-1 hover:bg-gray-100"
            >
              &lsaquo;
            </a>
          ...
          {Array.from({ length: Math.min(8, totalPages) }, (_, i) => {
            ...
            return (
              <a
                key={pageNum}
                href={`/board-extra1?page=${pageNum}${searchQuery ? `&search=${searchQuery}&searchType=${searchType}` : ''}`}
          ...
          {currentPage < totalPages && (
            <a
              href={`/board-extra1?page=${totalPages}${searchQuery ? `&search=${searchQuery}&searchType=${searchType}` : ''}`}
              className="rounded border px-3 py-1 hover:bg-gray-100"
            >
              last
            </a>
